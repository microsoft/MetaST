...
def readfile(filename):
    ...
    for line in f:
        ...
        sentence.append(splits[0]) ##TODO: add for lower case
...
class NerProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for i, (sentence, label) in enumerate(lines):
            ...
            text_a = sentence
            label = label
            examples.append(InputExample(guid=guid, words=text_a, label=label))
        return examples

#
# def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, pad_token=0, pad_label_token=-100):
...
#         label_ids.append(pad_label_token)
...
def convert_unlabeled_example_to_features(example, max_seq_length, tokenizer, pad_token=0, pad_label_token=-100):
    """
        ...
        if on_memory:
            ...
            with open(corpus_path, "r", encoding=encoding, errors='ignore') as f:
                for line in tqdm(f, desc="Loading Dataset", total=corpus_lines):
                    line = line.strip()
                    line = line ## TODO:add for lowercase
                    if line == "":
