...
def train_config(parser):
    ...
    parser.add_argument("--eval_dir",
                        ...
                        type=str,
                        required=True,
                        help="The input eval dir. Should contain the .tsv files (or other data files) for the task.")
    parser.add_argument("--test_dir",
                        default='test.txt',
                        type=str,
                        required=True,
                        help="The input test dir. Should contain the .tsv files (or other data files) for the task.")                    
...
def train_epoch(model, train_examples, label_list,tokenizer,num_train_optimization_steps,device,n_gpu,optimizer,args,processor):
    ...
    for _ in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        eval_result =  eval_epoch(model, label_list, tokenizer, device, args,processor,'validation')
    ...
    #track azureml log
    best_val_f1 = eval_epoch(model, label_list, tokenizer, device, args,processor,'validation')
    run.log('best_val_f1', np.float(best_val_f1))


def eval_epoch(model,label_list,tokenizer,device,args,processor,mode):
    # updated to test
    # better add training log
    # eval_examples = processor.get_dev_examples(args.data_dir)
    if mode=='test':
        #eval_examples = processor.get_test_examples(args.data_dir)
        eval_examples = processor.get_eval_examples(args.test_dir)
    else:
        #eval_examples = processor.get_dev_examples(args.data_dir)
        eval_examples = processor.get_eval_examples(args.eval_dir)
...
def main():
    ...
    if args.do_train:
        ...
        #To do: better module
        train_epoch(model, train_examples, label_list, tokenizer, num_train_optimization_steps,device,n_gpu,optimizer,args,processor)
        # add evaluation for each epoch
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        eval_epoch(model, label_list, tokenizer, device, args,processor,'test')
