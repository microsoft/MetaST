...
DATA_ROOT=$PROJECT_ROOT/dataset/cortana/
...
SHOT=10
SCENARIO=outlook
cat $DATA_ROOT/calendar.labeled_0.1_Train_en-us.bio.txt > $DATA_ROOT/train.txt
cat $DATA_ROOT/calendar.unlabeled_Train_en-us.bio.txt > $DATA_ROOT/unlabeled_train.txt
cat $DATA_ROOT/calendar.labeled_Dev_en-us.bio.txt > $DATA_ROOT/dev.txt
cat $DATA_ROOT/calendar.labeled_Test_en-us.bio.txt > $DATA_ROOT/test.txt
cat $DATA_ROOT/train.txt $DATA_ROOT/unlabeled_train.txt $DATA_ROOT/dev.txt $DATA_ROOT/test.txt | cut -d " " -f 2 | sort | uniq > $DATA_ROOT/labels.txt
# output
