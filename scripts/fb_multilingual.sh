...
DATA_ROOT=$PROJECT_ROOT/dataset/multilingual/FB_slot/
...
python3 preprocess.py $DATA_ROOT/alarm.train_whole_100_es-es.txt $BERT_MODEL $MAX_LENGTH > $DATA_ROOT/train.txt
python3 preprocess.py $DATA_ROOT/alarm.unlabeled_train_whole_100_es-es.txt $BERT_MODEL $MAX_LENGTH > $DATA_ROOT/unlabeled_train.txt
python3 preprocess.py $DATA_ROOT/alarm.train_whole_en-us.txt $BERT_MODEL $MAX_LENGTH > $DATA_ROOT/src.txt
python3 preprocess.py $DATA_ROOT/alarm.dev_whole_es-es.txt $BERT_MODEL $MAX_LENGTH > $DATA_ROOT/dev.txt
python3 preprocess.py $DATA_ROOT/alarm.test_whole_es-es.txt $BERT_MODEL $MAX_LENGTH > $DATA_ROOT/test.txt

cat $DATA_ROOT/train.txt $DATA_ROOT/unlabeled_train.txt $DATA_ROOT/dev.txt $DATA_ROOT/src.txt $DATA_ROOT/test.txt | cut -d " " -f 2 | grep -v "^$"| sort | uniq > $DATA_ROOT/labels.txt
...
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=$GPUID python3 run_self_training_multilingual.py --data_dir $DATA_ROOT \
