...
from meta_controler import VNet
...
from eval import evaluate, get_product_metrics
...
def setup_wandb(model, ):
    ...
    wandb.init(project=os.getenv("WANDB_PROJECT", "huggingface"), config=vars(args))
    # keep track of model topology and gradients
    if os.getenv("WANDB_WATCH") != "false":
        wandb.watch(
            model, log=os.getenv("WANDB_WATCH", "gradients"), log_freq=max(100, logging_steps)
        )


# def log(logs: Dict[str, float], iterator: Optional[tqdm] = None) -> None:
...
def build_controler(args, config):
    vnet = VNet(config, args)
    vnet.to(args.device)
    if args.n_gpu > 1:
        vnet = torch.nn.DataParallel(vnet)
    optimizer = torch.optim.Adam(vnet.parameters(), lr=args.learning_rate)
    vnet.zero_grad()
    return vnet, optimizer
...
def initialize(args, model_class, config, t_total, step, model_folder=None):
    # if is_wandb_available():
    #     self._setup_wandb()
    # else:
    #     logger.info(
    ...
    if step == 0:
        ...
        if os.path.isfile(os.path.join(model_folder, "optimizer.pt")) and os.path.isfile(
                os.path.join(model_folder, "scheduler.pt")) and os.path.isfile(os.path.join(model_folder, "model.pt")):
            # Load in optimizer and scheduler states
            optimizer.load_state_dict(torch.load(os.path.join(model_folder, "optimizer.pt")))
            scheduler.load_state_dict(torch.load(os.path.join(model_folder, "scheduler.pt")))
            model.load_state_dict(torch.load(os.path.join(model_folder, "model.pt")))
    if args.fp16:
        try:
...
def train_meta_controler(args,
    ...
    for i in range(sampling_step):
        ...
        if args.use_vnet:
            loss_weight = vnet(**inputs, logits=logits, final_emb=final_embeds, tau=1).squeeze()
            new_loss = (loss.sum(-1) * loss_weight).sum() / (loss_weight.sum()) + la_loss
            fast_weights = update_parameters(self_training_fast_model, new_loss, step_size=args.learning_rate)
            ...
            # if delta % 10 == 0:
            #     pdb.set_trace()
            # if sampling_strategy == "loss":
            #     meta_sampler.async_update_matrix(eval_batch[-1], eval_batch[3], logits)
            ##########################################

            if args.use_meta_vat == 1:
                vat_loss = vat_meta(args, config, self_training_fast_model, optimizer, final_embeds, logits,
                                    fast_weights, eval_batch)
                loss += args.vat_beta * vat_loss
    ...
    return vnet, loss.data, loss_weight.detach()


def vat_meta(args, config, model, optimizer, final_embeds, logits, fast_weights, labeled_batch):
    vat_loss = 0
    ...
    if not word_embed:
        ...
        embeds = word_embed(labeled_batch[0])
        vat_embeds = (
                embeds.data.detach() + embeds.data.new(embeds.size()).normal_(0, 1) * 1e-5).detach()
        vat_embeds.requires_grad_()

        vat_inputs = {"inputs_embeds": vat_embeds, "attention_mask": labeled_batch[1], "labels": labeled_batch[3]}
        if args.model_type != "distilbert":
            vat_inputs["token_type_ids"] = (
                labeled_batch[2] if args.model_type in ["bert", "xlnet"] else None
            )  # XLM and RoBERTa don"t use segment_ids

        vat_outputs = model(**vat_inputs, args=args)
        ...
        if (torch.isnan(norm) or torch.isinf(norm)):
            ...
            vat_outputs = functional_bert(fast_weights, config, **vat_inputs, args=args)
            vat_logits, vat_final_embeds = vat_outputs[1], vat_outputs[2]
            if args.vat_loss_type == "embeds":
...
def train_meta_Psuedo_label(args,
                            config,
                            self_training_teacher_model,
                            teacher_optimizer,
                            student,
                            input_list,
                            batch,
    ...
    if sampling_step < 0:
        sampling_step = len(meta_dataloader)
    total_loss = 0
    self_training_fast_model = student
    softmax = torch.nn.Softmax(dim=1)
    self_training_teacher_model.train()
    eval_batch = next(meta_dataloader.__iter__())
    eval_batch = tuple(t.to(args.device) for t in eval_batch)
    eval_inputs = {"input_ids": eval_batch[0], "attention_mask": eval_batch[1],
                   "labels": eval_batch[3]}
    ...
    for i in range(sampling_step):
        total_loss = 0
        if args.use_MPL in [1, 3]:
            outputs = self_training_teacher_model(**un_inputs, args=args, tau=tau)
            hard_label = outputs[2]
            pred_labels = hard_label.view(-1, hard_label.shape[-1]).view(hard_label.shape)

            MPL_inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": pred_labels,
                          "label_mask": None}

            outputs = self_training_fast_model(**MPL_inputs)
            loss, logits, final_embeds = outputs[0], outputs[1], outputs[2]
            ...
            loss = loss + args.labeled_beta * la_loss
            if args.n_gpu > 1:
                loss = loss.mean()
            fast_weights = update_parameters(self_training_fast_model, loss, step_size=args.controler_step_size)

            outputs = functional_bert(fast_weights, config, **eval_inputs, args=args)
            total_loss = outputs[0]
        if args.use_MPL in [2, 3]:
            outputs = self_training_teacher_model(**eval_inputs, args=args, tau=tau)
            total_loss += outputs[0]
        if args.n_gpu > 1:
            total_loss = total_loss.mean()
        total_loss.backward()
        teacher_optimizer.step()
        self_training_teacher_model.zero_grad()
        self_training_fast_model.zero_grad()
...
def finetune_model(args, config, model_folder, model, optimizer, scheduler, train_dataloader, model_class, tokenizer,
                   labels,
                   pad_token_label_id, learning_rate, best_test, best_dev, global_step,
    ...
    for i in tqdm(range(steps)):
        ...
        outputs = model(**inputs, args=args, use_focal_loss=args.use_focal_loss)
        loss = outputs[0]
        if args.n_gpu > 1:
        ...
        if (i + 1) % args.logging_steps == 0:
            ...
            output_dirs = []
            if args.local_rank in [-1, 0] and test_updated:
                output_dirs.append(os.path.join(args.output_dir, "test-best"))
...
def train(args, model_class, config, tokenizer, labels,
    ...
    model, optimizer, scheduler = initialize(args, model_class, config, args.self_training_begin_step, 0)
    # Train!
    ...
    if os.path.exists(args.model_name_or_path):
        ...
        if 'checkpoint' not in args.model_name_or_path:
            logger.info("  loading from pretrained language model weight")
    ...
    loss, mt_loss, vat_loss = 0.0, 0.0, 0.0
    epoch = 0

    # Check if pre-empt, then continue training from a most recent checkpoint
    model_folder = ''
    for f_name in files:
        if "checkpoint" in f_name:
            try:
                tmp_global_step = int(f_name.split("-")[-1].split("/")[0])
            except:
                continue
            if tmp_global_step > global_step:
                model_folder=  os.path.join(args.output_dir, "test-best")
    ...
    if global_step > 0:
        model, optimizer, scheduler = initialize(args, model_class, config, args.self_training_begin_step, -1, model_folder)

        logger.info("***** Global_step: %d, Entropy loss: %.4f, mean teacher loss : %.4f; vat loss: %.4f *****", \
                    global_step, loss - args.mt_beta * mt_loss - args.vat_beta * vat_loss, \
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.evaluate_during_training:
                        ...
                        # get_product_metrics(args, c_result)
                        for key, value in results.items():
                            tb_writer.add_scalar("test_{}".format(key), value, global_step)
    ...
    vnet, v_optimizer = build_controler(args, config)
    ...
    if global_step >= args.self_training_begin_step:
        ...
        for step in tqdm(range(args.self_training_begin_step, args.max_steps)):
            ...
            tau = max(0.5, math.exp(-delta * 1.0 / args.self_training_period))



            if args.gardual_training == 1:
                n_level = (delta % args.self_training_period) // args.gardual_training_period
                self_training_hp_label = max(args.self_training_hp_label - 0.1 * n_level,
                                             args.self_training_hp_label - 1)

            if delta % args.self_training_period == 0:
                # self_training_teacher_model, teacher_optimizer =  get_optimizer(args,
                #                                                                 self_training_teacher_model,
                ...
                if len(meta_dataset) != 0:
                    # meta_sampler = RandomSampler(meta_dataset) if args.local_rank == -1 else DistributedSampler(
                    #     meta_dataset)
            ...
            if args.use_MPL != 0:
                if teacher_updated:
                    MPL_loss = train_meta_Psuedo_label(args,
                                                       config,
                                                       self_training_teacher_model,
                                                       teacher_optimizer,
                                                       student=model,
                                                       input_list=[labeled_inputs, unlabeled_inputs],
            ...
            if args.model_type != "distilbert":
                unlabeled_inputs["token_type_ids"] = (
                    unlabeled_batch[2] if args.model_type in ["bert", "xlnet"] else None
                )

            if args.use_controler:
            ...
            if args.use_controler:
                vnet, meta_loss, loss_weight = train_meta_controler(args, config, model, optimizer,
                                                                    [labeled_inputs, unlabeled_inputs],
                                                                    meta_dataloader,  vnet, v_optimizer,
                                                                    sampling_step=args.controler_sampling_steps)
                if not args.use_vnet:
                    ...
                    if args.use_labeled_data == 2:
                        ...
                        # print(1 - (loss_weight == 0).sum() * 1.0 / loss_weight.size(0))
                    else:
                        # print(1- (loss_weight == 0).sum()*1.0/loss_weight.size(0))
                    ...
                    if args.self_training_label_mode != "hard":
                        loss = un_loss.sum(-1)
                    # print((loss_weight).sum() * 1.0 / loss_weight.size(0))
                    loss = (un_loss * loss_weight).sum()
            ...
            if args.mt and global_step % args.mt_updatefreq == 0:
                ...
                if update_step == 1:
                    ...
                    alpha = args.mt_alpha2
                mt_update(teacher_model.named_parameters(), model.named_parameters(), args.mt_avg, alpha,
                          update_step)

            if args.mt and update_step > 0:
                with torch.no_grad():
                    teacher_outputs = teacher_model(**labeled_inputs)
                    teacher_logits, teacher_final_embeds = teacher_outputs[1], teacher_outputs[2]
            ...
            if args.vat in [2, 3]:
                ...
                if not word_embed:
                    ...
                    vat_inputs = {"inputs_embeds": vat_embeds, "attention_mask": labeled_batch[1],
                                  "labels": labeled_batch[3]}
                    if args.model_type != "distilbert":
                        labeled_inputs["token_type_ids"] = (
                            labeled_batch[2] if args.model_type in ["bert", "xlnet"] else None
                        )  # XLM and RoBERTa don"t use segment_ids
            ...
            if args.use_labeled_data == 2:
                ...
                loss = loss + args.mt_beta * mt_loss + args.vat_beta * vat_loss + args.labeled_beta * labeled_loss
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    print(meta_sampler.weights)
                    print(meta_sampler.all_values)

                    if args.evaluate_during_training:

                        logger.info(
                            "***** Global_step: %d, Entropy loss: %.4f, mean teacher loss : %.4f; vat loss: %.4f; meta loss : %.4f *****", \
                            global_step, loss - args.mt_beta * mt_loss - args.vat_beta * vat_loss, \
                            args.mt_beta * mt_loss, args.vat_beta * vat_loss, meta_loss)
...
def main():
    ...
    parser.add_argument('--split_ratio', default=0.5, type=float, help="labeled data : meta data.")
    parser.add_argument('--split_mode', default='random', type=str, help="coefficient of labeled_loss term.")

    parser.add_argument('--gardual_training', type=int, default=0,
                        help='0: dont use meta_vat loss, 1: use meta_vat loss.')

    # Meta Controler
    parser.add_argument('--use_controler', action="store_true", help='use controler or not.')

    parser.add_argument('--self_controler_period', type=int, default=1,
                        help='the controler period.')
    parser.add_argument('--use_gumbel', action="store_true", help='Use Gumbel Softmax.')
    parser.add_argument('--use_vnet', action="store_true", help='Use Vnet or pure weight.')
    ...
    parser.add_argument('--data_scenario', type=str, default='outlook', help='outlook or mobile.')
    parser.add_argument('--use_product', action="store_true", help='use product_metrics or not.')
    parser.add_argument('--per_gpu_meta_batch_size', type=int, default=8, help='use product_metrics or not.')
    parser.add_argument('--use_MPL', type=int, default=0,
                        help='3: meta psuedo label with supervision, 2: only supervision, 1：use Meta Psuedo Label no supervision， 0： dont use Meta Psuedo.')
    parser.add_argument('--use_teacher_supervision', type=int, default=0,
                        help='use supervision to train teacher or not.')
    parser.add_argument('--teacher_mpl_start_steps', type=int, default=1000, help='use Meta Psuedo Label or not.')
    ...
    parser.add_argument('--use_meta_vat', type=int, default=0,
                        help='0: dont use meta_vat loss, 1: use meta_vat loss.')
    ...
    # virtual adversarial training
    parser.add_argument('--vat', type=int, default=0,
                        help='0: no vat, 1: stage 1, 2: stage 2: 3: both virtual adversarial training.')
    parser.add_argument('--vat_eps', type=float, default=5e-5,
    ...
    parser.add_argument('--vat_beta', type=float, default=0.1,
    ...
    #                     help='perturbation size for virtual adversarial training.')
    # parser.add_argument('--vat_lambda', type=float, default=1,
    #                     help='trade off parameter for virtual adversarial trainin2.')
    ...
    parser.add_argument('--self_student_period', type=int, default=20000, help='the self-training period.')
    parser.add_argument('--sampling_begin_step', type=int, default=1, help='the self-training period.')
    parser.add_argument('--sampling_update_freq', type=int, default=1, help='the self-training period.')
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
        model_path = os.path.join(args.output_dir, "test-best")
